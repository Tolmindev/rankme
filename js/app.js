...
function hsl(t, extra){
  extra = extra || {};
  const l = extra.light !== undefined ? extra.light : t.light;
...
function render(){
  ...
  state.tiers.forEach((t, idx)=>{
    ...
    label.style.background = `linear-gradient(180deg, ${hsl(t,{light:t.light+6>95?95:t.light+6})}, ${hsl(t)})`;
    label.style.color = textColorFor(t);
    ...
    side.innerHTML = `
      <button class="gear" title="Настройки ряда">&#9881;</button>
      <button class="up" title="Вверх" ${idx===0?'disabled style="opacity:.25;pointer-events:none"':''}>&#9650;</button>
      <button class="down" title="Вниз" ${idx===state.tiers.length-1?'disabled style="opacity:.25;pointer-events:none"':''}>&#9660;</button>
    `;
    row.appendChild(side);

    side.querySelector('.gear').addEventListener('click', (e)=> openRowSettings(t.id, side.querySelector('.gear')));
...
function renderPool(){
  ...
  poolCountEl.textContent = `${state.pool.length} / ${N_CARDS} персонажей`;
}
...
function makeCard(cid){
  ...
  el.title = meta ? `${meta.name} — ${meta.faction}` : '';
  el.appendChild(img);
...
function renderFactionFilters(){
  ...
  mkBtn('ALL', 'Все');
  FACTIONS.forEach(f => mkBtn(f, f, FACTION_HUE[f] ?? 220));
}
...
function onDragMove(e){
  ...
  if(cont){
    ...
    const children = [...cont.children].filter(c=>!c.classList.contains('placeholder'));
    let inserted = false;
...
function onDragEnd(e){
  ...
  if(drag.targetContainer){
    ...
    const ids = [...cont.children].map(c => c.classList.contains('placeholder') ? cid : parseInt(c.dataset.cardId));
    ...
    // dropped nowhere valid -> return to origin
    if(drag.startedContainer.classList.contains('pool')) state.pool.push(cid);
    else {
...
function openRowSettings(tierId, anchorBtn){
  ...
  pop.innerHTML = `
    <label>Цвет ряда</label>
    <input type="range" class="hue-slider" min="0" max="360" value="${t.hue}">
    <label>Название ряда</label>
    <input type="text" value="${t.name}" maxlength="20">
    <div class="rs-btns">
      <button class="clear">Очистить</button>
      <button class="danger del">Удалить</button>
    </div>
  `;
  anchorBtn.closest('.tier-row').appendChild(pop);
  openPopover = pop;

  pop.querySelector('.hue-slider').addEventListener('input', (e)=>{
    t.hue = parseInt(e.target.value);
    render();
    // re-open on the same row after re-render
    const newRow = document.querySelector(`.tier-row[data-tier-id="${tierId}"]`);
    if(newRow){ newRow.appendChild(pop); openPopover = pop; }
  });
  pop.querySelector('input[type=text]').addEventListener('input', (e)=>{
    t.name = e.target.value.toUpperCase();
    const lbl = document.querySelector(`.tier-row[data-tier-id="${tierId}"] .tier-label`);
  ...
  pop.querySelector('.del').addEventListener('click', ()=>{
    if(state.tiers.length<=1){ showToast('Нужен хотя бы один ряд'); return; }
    state.pool.push(...(state.assignment[tierId]||[]));
...
document.getElementById('addRowBtn').addEventListener('click', ()=>{
  ...
  state.tiers.push({id, name:'НОВЫЙ РЯД', hue:200, sat:50, light:60});
  state.assignment[id] = [];
  render();
});
...
document.getElementById('shareDiscord').addEventListener('click', ()=>{
  ...
  navigator.clipboard?.writeText(url).then(()=> showToast('Ссылка скопирована — вставь в Discord'))
    .catch(()=> showToast(url));
});
document.getElementById('shareTelegram').addEventListener('click', ()=>{
  const url = buildShareUrl();
  window.open(`https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent('Мой тирлист Street Fighter: Duel на RankMe')}`, '_blank');
});
document.getElementById('shareX').addEventListener('click', ()=>{
  const url = buildShareUrl();
  window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent('Мой тирлист Street Fighter: Duel на RankMe 🥊')}&url=${encodeURIComponent(url)}`, '_blank');
});
...
async function exportPNG(){
  ...
  const rowH = 110;
  const padTop = 40;
  const height = padTop + state.tiers.length * rowH + 60;
  ...
  for(const t of state.tiers){
    ...
    ctx.fillStyle = t.light>60 ? '#1a1620' : '#ffffff';
    ctx.font = 'bold 20px sans-serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    wrapText(ctx, t.name, 65, y+rowH/2, 118, 22);
  ...
  ctx.textAlign = 'left';
  ctx.fillText('RANKME.LOL — create tier lists in seconds', 10, height-20);
