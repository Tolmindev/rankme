...
const CARD_META_LIST = (window.RANKME_BLANK || (TMPL && TMPL.id === 'blank'))
  ? ((TMPL && Array.isArray(TMPL.cards)) ? TMPL.cards : [])
  : ((TMPL && Array.isArray(TMPL.cards) && TMPL.cards.length) ? TMPL.cards : DEFAULT_CARD_META_LIST);
const CARD_META = {};
...
if(BLANK_MODE){
  ...
  // Blank: show Upload Images (same control as remix upload)
  const uploadWrap = document.getElementById('remixUploadWrap');
  if(uploadWrap) uploadWrap.hidden = false;
  const upload = document.getElementById('remixUpload') || document.getElementById('uploadImgs');
  if(upload && !upload._rankmeBound){
    upload._rankmeBound = true;
